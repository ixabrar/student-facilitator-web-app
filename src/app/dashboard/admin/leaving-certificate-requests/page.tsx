...
export default function AdminLeavingCertificateManagementPage() {
  ...
  const fetchCertificates = async () => {
    try {
      const url = statusFilter === 'all' ? '/api/leaving-certificates' : `/api/leaving-certificates?status=${statusFilter}`
  ...
  const handleIssue = async (cert: LeavingCertificate) => {
    ...
    try {
      const res = await fetch(`/api/leaving-certificates/${cert._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          status: 'issued',
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Leaving Certificate (LC) Requests</h1>
        <p className="text-muted-foreground">Review and process student leaving certificate requests</p>
      </div>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <CardDescription>
                      Email: {cert.studentEmail} | Reason: {cert.reason}
                    </CardDescription>
                  </div>
              ...
              <CardContent className="space-y-4">
                ...
                {cert.status === 'approved' && (
                  <div className="space-y-2 pt-2">
                    <Label htmlFor={`lc-url-${cert._id}`}>Certificate URL or File Link</Label>
                    <div className="flex gap-2">
                      <Input
                        id={`lc-url-${cert._id}`}
                        placeholder="https://..."
