...
export default function BonafidePage() {
  ...
  const fetchCertificates = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/bonafide-certificates?studentId=${profile._id}`, {
  ...
  const handleRequest = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('authToken')
      const res = await fetch('/api/bonafide-certificates', {
        method: 'POST',
        headers: { 
      ...
      toast.success('Bonafide certificate request submitted!')
      setPurpose('')
      setDialogOpen(false)
      fetchCertificates()
    } catch (error: any) {
  ...
  const handleCancel = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/bonafide-certificates/${id}`, {
        method: 'DELETE'
      })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Bonafide Certificate</h1>
          <p className="text-muted-foreground">Request and track your bonafide certificates</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Request Bonafide Certificate</DialogTitle>
              <DialogDescription>Provide details for your certificate request</DialogDescription>
            </DialogHeader>
            <form onSubmit={handleRequest} className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea
                  id="purpose"
                  placeholder="e.g., Scholarship application, Admission to higher studies, Employment verification..."
                  value={purpose}
              ...
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <CardTitle className="text-lg">Bonafide Certificate Request</CardTitle>
                    <CardDescription>
