...
import { LeavingCertificate } from '@/lib/db/models'
...
export async function PUT(request: NextRequest, props: { params: Promise<{ certId: string }> }) {
  try {
    ...
    const cert = await LeavingCertificate.findById(params.certId)
    if (!cert) {
...
export async function DELETE(request: NextRequest, props: { params: Promise<{ certId: string }> }) {
  try {
    ...
    await LeavingCertificate.findByIdAndDelete(params.certId)
